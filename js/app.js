...
export async function startApp(build){
  ...
  RIVER=base.RIVER;MAP_REFS=base.MAP_REFS;
  try{const custom=await import(`../data/custom-landmarks.js?v=${encodeURIComponent(BUILD)}`);applyCustomData(custom)}catch(e){console.warn('Custom landmark layer unavailable',e)}
  configureBranchRoutes();
  try{const b=await import(`../data/bridges.js?v=${encodeURIComponent(BUILD)}`);BRIDGES=b.BRIDGES||[]}catch(e){console.warn('Bridge layer unavailable',e)}
  bindUI();renderList();renderCruise();
}
...
function applyCustomData({LANDMARK_OVERRIDES={},LANDMARK_ADDITIONS=[],ROUTE_INSERTS=[]}){
  ...
  for(const e of ROUTE_INSERTS||[]){const arr=TOUR_ORDERS?.[e.route]?.[e.direction];if(!arr||arr.includes(e.id))continue;let i=e.after?arr.indexOf(e.after)+1:e.before?arr.indexOf(e.before):arr.length;if(i<0)i=arr.length;arr.splice(i,0,e.id)}
}
function configureBranchRoutes(){TOUR_ORDERS={
  ...
  south:{outbound:['wolf-point','150-riverside','civic-opera','300-wacker','willis-tower','old-post-office','river-city','st-charles-bridge'],return:['st-charles-bridge','river-city','old-post-office','cbot','300-wacker','willis-tower','civic-opera','150-riverside','wolf-point']},
...
function renderCruise(){
  const items=routeItems();if(!items.length)return;if(currentIndex>=items.length)currentIndex=0;if(currentIndex<0)currentIndex=items.length-1;
  ...
  $('#heroMoreBtn').onclick=()=>openDetail(l.id);const n=items[(currentIndex+1)%items.length];$('#nextName').textContent=n.name;$('#nextMeta').textContent=`${n.side}${userPos?' • '+fmtDist(distanceMeters(userPos,n)):''}`;renderMap();
}
function renderList(filter=''){const q=filter.toLowerCase();$('#landmarkList').innerHTML=LANDMARKS.filter(l=>!q||[l.name,l.story,l.architect].some(v=>(v||'').toLowerCase().includes(q))).map(l=>`<div class="landmark-item" data-id="${l.id}"><div class="landmark-thumb" style='${photoStyle(l)}'></div><div><h3>${esc(l.name)}</h3><p>${esc(l.style)} • ${esc(l.year)}</p></div><button aria-label="Open ${esc(l.name)}">›</button></div>`).join('');$$('.landmark-item').forEach(el=>el.onclick=()=>openDetail(el.dataset.id))}
function openDetail(id){const l=LANDMARKS.find(x=>x.id===id);if(!l)return;const things=thingsFor(l);$('#modalContent').innerHTML=`<div class="modal-photo" style='${photoStyle(l)}'></div><div class="modal-copy">${l.credit?`<div class="photo-credit">Photo: ${esc(l.credit)}</div>`:''}<span class="eyebrow">${esc(l.side).toUpperCase()}</span><h2>${esc(l.name)}</h2><p class="subtitle">${esc(l.subtitle)}</p><div class="things-box"><h3>Things to know</h3><ul>${things.map(x=>`<li>${esc(x)}</li>`).join('')}</ul></div><div class="fun-box"><h3>★ Fun fact</h3><p>${esc(l.fact)}</p></div><div class="detail-grid"><div><strong>Architect</strong><span>${esc(l.architect)}</span></div><div><strong>Completed</strong><span>${esc(l.year)}</span></div><div><strong>Style</strong><span>${esc(l.style)}</span></div><div><strong>GPS</strong><span>${l.lat.toFixed(4)}, ${l.lng.toFixed(4)}</span></div></div></div>`;$('#detailModal').classList.add('open');$('#detailModal').setAttribute('aria-hidden','false')}
function closeDetail(){$('#detailModal').classList.remove('open');$('#detailModal').setAttribute('aria-hidden','true')}

function activeRiverPoints(){return RIVER[currentRoute]}
function allRiverPoints(){return RIVER.main.concat(RIVER.south.slice(1),RIVER.north.slice(1))}
function mapBoundsFor(points){const lats=points.map(p=>p[0]),lngs=points.map(p=>p[1]);return{minLat:Math.min(...lats)-.0022,maxLat:Math.max(...lats)+.0022,minLng:Math.min(...lngs)-.0030,maxLng:Math.max(...lngs)+.0030}}
let MAP_BOUNDS=null;
...
function bridgeSvg(activeRiver){return BRIDGES.filter(b=>b.branch===currentRoute).map(b=>{const p=nearestProjection(b.lat,b.lng,activeRiver);if(!p)return '';const nx=-p.ty,ny=p.tx,half=3.6,x1=p.x-nx*half,y1=p.y-ny*half,x2=p.x+nx*half,y2=p.y+ny*half,s=b.labelSide||1,lx=p.x+nx*5.0*s,ly=p.y+ny*5.0*s,anchor=nx*s>.18?'start':nx*s<-.18?'end':'middle';return `<g class="map-bridge"><line class="bridge-shadow" x1="${x1}" y1="${y1}" x2="${x2}" y2="${y2}"></line><line class="bridge-deck" x1="${x1}" y1="${y1}" x2="${x2}" y2="${y2}"></line><text x="${lx}" y="${ly}" text-anchor="${anchor}">${esc(b.name)}</text></g>`}).join('')}
function renderMap(){
  const target=$('#map');if(!target)return;const ordered=routeItems(),activeRiver=activeRiverPoints();MAP_BOUNDS=mapBoundsFor(allRiverPoints());target.dataset.route=currentRoute;
  const context=['main','south','north'].filter(r=>r!==currentRoute).map(r=>`<path class="context-river" data-branch="${r}" d="${geoPath(RIVER[r])}"></path>`).join('');
  const river=`<path class="river-line" d="${geoPath(activeRiver)}"></path><path class="river-core" d="${geoPath(activeRiver)}"></path>`;
  const bridges=bridgeSvg(activeRiver),route=`<path class="route-line" d="${geoPath(activeRiver)}"></path>`;
...
function changeRoute(value){currentRoute=value;currentIndex=0;$('#routeSelect').value=value;if($('#mapRouteSelect'))$('#mapRouteSelect').value=value;renderCruise()}
function bindUI(){
  $('#gpsBtn').onclick=startGPS;$('#nextBtn').onclick=()=>{currentIndex++;renderCruise()};$('#prevBtn').onclick=()=>{currentIndex--;renderCruise()};
  $('#routeSelect').onchange=e=>changeRoute(e.target.value);
  $('#mapRouteSelect').onchange=e=>{const cruise=$('#routeSelect');cruise.value=e.target.value;cruise.dispatchEvent(new Event('change',{bubbles:true}))};
  ...
  $$('.bottom-nav button').forEach(b=>b.onclick=()=>{$$('.bottom-nav button').forEach(x=>x.classList.remove('active'));b.classList.add('active');$$('.view').forEach(v=>v.classList.remove('active'));$('#'+b.dataset.view).classList.add('active');if(b.dataset.view==='mapView'){if($('#mapRouteSelect'))$('#mapRouteSelect').value=currentRoute;renderMap()}});
